...
class _SplashPageState extends State<SplashPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _checkFirstLaunch() async {
    try {
      ...
      // 检查是否存在first_launch键
      final bool keyExists = prefs.containsKey('first_launch');
      final bool isFirstLaunch = prefs.getBool('first_launch') ?? true;
      
      debugPrint('SharedPreferences 状态: 键存在=${keyExists}, 首次启动=${isFirstLaunch}');
  ...
  void _navigateToNextScreen() async {
    try {
      ...
      debugPrint('导航到下一个屏幕: 首次启动=${_isFirstLaunch}');
      
      if (_isFirstLaunch) {
        // 如果是首次启动，跳转到引导页，并设置标志为false
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Text(
                      _isFirstLaunch ? '首次启动状态: 是' : '首次启动状态: 否',
                      style: TextStyle(
                        fontSize: 12,
